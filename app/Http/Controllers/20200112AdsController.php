    	...
    	if ($user == false) {
            ...
            if(isset($data['search']['value']) && $data['search']['value'] != null ) {
                ...
                $sql = "SELECT DISTINCT u.`UID`, CONCAT(u.`firstname`, ' ', u.`surname`) AS 'username', u.`Uemail`, am.`expiry`, am.`payment_exp_date`,am.`category`,am.`am`
                        ...
                        WHERE am.`type` = 'Member' AND (am.`payment_exp_date` < '" . $fourteen_days_before . "' OR am.`payment_exp_date` !='0000-00-00') AND 
                ...
                $sql = "SELECT DISTINCT u.`UID`, CONCAT(u.`firstname`, ' ', u.`surname`) AS 'username', u.`Uemail`, am.`expiry`, am.`payment_exp_date`,am.`category`,am.`am`
                        ...
                        INNER JOIN `users` AS u ON u.`UID` = am.`uid`
                        INNER JOIN `adverts` AS a ON a.`UID` = am.`uid` AND a.`is_active` = 1
                        WHERE (am.`type` = 'Member' || am.`type` = 'Expired') AND (am.`payment_exp_date` < '" . $fourteen_days_before . "' OR am.`payment_exp_date` ='0000-00-00') 
                        ORDER BY am.`payment_exp_date` DESC";
            }
    ...
    public function limitExceed() {
        if(isset($data['search']['value']) && $data['search']['value'] != null ) {
            ...
            $sql = "SELECT DISTINCT u.`UID`, CONCAT(u.`firstname`, ' ', u.`surname`) AS 'username', u.`Uemail`, am.`expiry`, am.`payment_exp_date`,am.`category`,am.`am`
                    ...
                    WHERE am.`type` != 'Member' OR ( am.`type` = 'Member' AND ap.`num_of_max_ads` < a.`ad_count` AND 
    ...
    public function nullADData() {
        ...
        if(isset($data['search']['value']) && $data['search']['value'] != null ) {
            ...
            $sql = "SELECT DISTINCT u.`UID`, CONCAT(u.`firstname`, ' ', u.`surname`) AS 'username', u.`Uemail`, am.`expiry`, am.`payment_exp_date`,am.`category`,am.`am`
                        ...
                        INNER JOIN `users` AS u ON u.`UID` = am.`uid`
                        INNER JOIN `adverts` AS a ON a.`UID` = am.`uid` AND a.`is_active` = 1
                        WHERE am.`type` = '' OR (am.`type` IS NULL) OR am.`payment_exp_date` = '' OR (`payment_exp_date` IS NULL) AND  
                        (`users`.`firstname` LIKE '%".$search_data."%' OR `users`.`surname` LIKE '%".$search_data."%'  OR `users`.`Uemail` LIKE '%".$search_data."%'  
                        OR am.`am` LIKE '%".$search_data."%' OR am.`category` LIKE '%".$search_data."%' )
                        ORDER BY am.`payment_exp_date` DESC";
        } else {
            $sql = "SELECT DISTINCT u.`UID`, CONCAT(u.`firstname`, ' ', u.`surname`) AS 'username', u.`Uemail`, am.`expiry`, am.`payment_exp_date`,am.`category`,am.`am`
                        ...
                        INNER JOIN `users` AS u ON u.`UID` = am.`uid`
                        INNER JOIN `adverts` AS a ON a.`UID` = am.`uid` AND a.`is_active` = 1
                        WHERE am.`type` = '' OR (am.`type` IS NULL) OR am.`payment_exp_date` = '' OR (`payment_exp_date` IS NULL)
                        ORDER BY am.`payment_exp_date` DESC";
        }
        $customer = DB::select($sql);
    ...
    public function memberDashboardStoreData()
    {
        ...
        try {
            ...
            foreach ($member_data AS $pro => $value) {
                ...
                $output[$pro] = $value;
                //For get max page id
                $sql1 = "SELECT MAX(a.`current_page_id`) as 'max_page_id' 
                        FROM `adverts` AS a
                        WHERE a.`UID` = ".$uid."
                        ORDER BY a.`UID` DESC";
                ...
                $sql2 = "SELECT COUNT(a.`ad_id`) as 'pics_count' 
                        FROM `adverts` AS a
                ...
                //For get ads boosts
                $sql4 = "SELECT ca.`avail_balance`, cau.`last_upgraded_by` 
                ...
                $sql5 = "SELECT COUNT(cpl.`log_id`) as 'boosts' 
                ...
                /*$sql6 = "SELECT SUM(cs.`tel_contacts`) as 'tel', SUM(cs.`email_contacts`) as 'email', SUM(cs.`sms_contacts`) as 'sms'
                ...
                /*$sql8 = "SELECT SUM(ams.`total_views_per_month`) as 't_views', SUM(ams.`total_leads_per_month`) as 't_leads'
                        FROM `admin_members_stats` AS ams 
